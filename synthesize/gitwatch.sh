...
#   - Dominik D. Geyer
#
#  ADJUSTMENT: uses mock_inotifywait.sh which scans for changed .wks files in git status
#
#############################################################################
...
if [ -z "$GW_INW_BIN" ]; then INW="./mock_inotifywait.sh"; else INW="$GW_INW_BIN"; fi
